...
public:
	...
	T pop(){//??????? 
		T x=*top;
		if(top==(node->data)&&node->prev!=NULL){//????????,???? 
			node=node->prev;
			delete node->next;
			node->next=NULL;
...
double Fail(double a,double b){//????Fail??,?????????Fail?????term?? 
	return -1;
}
...
void tagCopy(char* dest,char* src,int n=nameLength){//?????,??function??????,??????????function????tag 
	for(int i=0;i<n-1;i++){
		dest[i]=src[i];
	}
	dest[n-1]='\0';
...
private:
	...
	struct termList{//?term??????????? 
		...
		errorInfo mergeAdjacent(termNode* t){//????t?????????????????? 
			termNode* begin;//???????? 
			termNode* end;//???????? 
			termNode* del;//?????? 
			termNode* del2=NULL;//?????? 
			...
			int i1,i2;//?????? 
			double d1,d2;//??????? 
			switch((t->data).category){//????category????,?????? 
				case term::L:
					del=t;
					begin=t->prev;
					end=t->next;//??????????????? 
	...
	struct matchTree{//??????,?????????? 
		...
		void createNode(treeNode*& t,function f[],int lower,int upper){
			...
			if(lower<=upper){
				t=new treeNode;
				mid=(lower+upper)/2;//????????,???????????????,??????????? 
				tagCopy(t->func.tag,f[mid].tag);//???????function??????tag 
				t->func.opr.attribute=f[mid].opr.attribute;//??function???????? 
				t->func.opr.category=f[mid].opr.category;
				switch(t->func.opr.attribute){//?function???????? 
					case term::biDouble:
		...
		matchTree(function f[],int n){//????????createNode 
	...
	struct val{//isSet???????????????????,value????,???????????? 
		bool isSet=false;
		double value;
	};
	
	void readNumber(char* ch,int& i,expression& e,bool omit=false){//?????????,??????e?,omit?????????????? 
		int ival=0;
		int orig;
		if(!omit){//?????????? 
			orig=i;
	...
	int readFunction(char* ch,int& i,expression& e,int offset){//??????????? 
		...
		if(rlt==-1){//???-1??readParameter??????calculate???????????,????????,??????calculate?????? 
			return -1;
		}
		else if(rlt==-2){//??readParameter????calculate????????,??????????,????calculate????????? 
			return -2;
		}
		return 0;
	}
	...
	term match(char* ch,int& i){//????? 
		...
		while(isAlphabet(ch[i])||node->func.tag[j]!='\0'){//???????????tag???????????? 
			while(node!=NULL){
				if(!isAlphabet(ch[i])){//???????????,tag???????????,???????sin?tag?sinh,????????******\0,\0????????,??????????????? 
					i=orig;
					j=0;
					node=node->lch;
				}
				else if(ch[i]>node->func.tag[j]){//?????,i,j????????? 
					i=orig;
					j=0;
					node=node->rch;
				}
		...
		else if(!isAlphabet(ch[i])&&node->func.tag[j]=='\0'){//???????,tag????????,?????? 
			term tmp;
			tmp.attribute=node->func.opr.attribute;
			tmp.category=node->func.opr.category;
			tmp.position=orig;
			switch(node->func.opr.attribute){//?function?????????term?? 
				case term::biDouble:
	...
	int readParameter(char* ch,int& i,expression& e,term* t,int begin,int offset){//?????? 
		if(ch[i]=='\0'){
			...
			else if(t->attribute==term::Array){//?????????????????1024 
				paraNum=1024;
			}
			double para[paraNum];//?????? 
			int paraCnt;//??????????? 
			bool integral=false;//?????????????? 
			val intVal[paraNum][3];//??????????????? 
			if(ch[i+1]==')'){//??????,???????0 
				paraCnt=0;
			}
			else{
			...
			if(paraCnt<paraNum&&paraNum!=1024){//???????????,???????????????? 
				err=true;
				errCode=16;
				return -1;
			}
			switch(t->attribute){//???????????????????? 
				...
				case term::uniInt:
					...
					break;
				case term::Array:
					if(equal(t->val.ptr5(para,paraCnt),(int)t->val.ptr5(para,paraCnt))){//????????????? 
...
public:
	calculator(){//calculator????? 
		tagCopy(functionTable[0].tag,"abs");//?????,??tag???????????? 
		...
		fTree=new matchTree(functionTable,20);//?????????????????? 
	}
	~calculator(){//???? 
		delete fTree;
	}
	result calculate(char* ch,int offset=0){//????????????????,????????O(n),???O(n^2),????????O(n),???O(n^2) 
		err=false;
		errCode=-1;//????? 
		if(ch[0]=='\0'){//??????0 
			result tmp;
		...
		if(offset==0){//??,??offset?0,???calculate???????????,???????????,???????,????????,????? 
			cout<<"=";
			e->print();
		}
		result tmp;//???????,?????????? 
